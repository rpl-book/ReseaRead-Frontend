...
const BookGroupDetail = ({ data, category }) => {
  ...
  return (
    <div>
      <p>{category}</p>
      <p className="fss mtr">
        {category} books with twist endings OR good twists and turns throughout.
      </p>
      {data.map((item, index) => (
        <div
          key={"span_parent_" + index}
          className="grid grid-cols-4 gap-3 mt2r"
        >
          <div
            key={"span_1_" + index}
          ...
          <div
            ...
            width={100}
            height={100}
            className="col-span-2 font-bold cursor-pointer"
            onClick={() => {
              router.push("/bookdetail");
            }}
          >
          ...
          <div
            key={"span_4_" + index}
            className="col-span-3 border-t border-gray-300 gap-3"
          ></div>
        </div>
      ))}
    </div>
